    ...
    <section id="meni" className="scroll-mt-20 py-20 sm:py-28">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <Tabs defaultValue={menu[0].id} className="mt-10 gap-6">
          <TabsList className="no-scrollbar mx-auto h-auto max-w-full flex-nowrap gap-1 overflow-x-auto rounded-full bg-secondary/70 p-1.5">
            {menu.map((cat) => (
              <TabsTrigger
                key={cat.id}
                value={cat.id}
                className="rounded-full px-5 py-2.5 text-sm font-semibold whitespace-nowrap text-muted-foreground transition-colors hover:text-foreground data-active:bg-terracotta! data-active:text-white! data-active:shadow-sm"
              >
                {cat.name}
              </TabsTrigger>
            ))}
